...
func miniMaxH3WorkflowManifest() workflowManifest {
	return workflowManifest{
		...
		Branches: []workflowBranchManifest{
			...
			{ID: "low_vram_attention", Name: "Low VRAM Attention", Description: "Экспериментальное разбиение attention по группам голов.", ToggleParameter: "video_low_vram_attention", RequiredClasses: []string{"MiniMaxLowVRAMAttention"}, Order: 20},
			{ID: "chunk_feed_forward", Name: "Chunk FeedForward", Description: "Экспериментальное разбиение feed-forward по токенам.", ToggleParameter: "video_chunk_feed_forward", RequiredClasses: []string{"MiniMaxChunkFeedForward"}, Order: 30},
			{ID: "sage_attention", Name: "SageAttention", Description: "Патч внимания MiniMax H3.", ToggleParameter: "video_sage_attention", RequiredClasses: []string{"MiniMaxH3MemoryEfficientSageAttentionPatch"}, Order: 40},
			{ID: "memory_optimization", Name: "H3 Memory Optimization", Description: "Ограничение пиковых активаций и управление точностью.", ToggleParameter: "video_memory_optimize", RequiredClasses: []string{"H3MemoryOptimization"}, Order: 50},
			{ID: "aimdo", Name: "AIMDO Residency Limiter", Description: "Ограничение резидентных блоков модели.", ToggleParameter: "video_aimdo_enabled", RequiredClasses: []string{"H3AIMDOResidencyLimiter"}, Order: 60},
			{ID: "sparse_attention", Name: "H3 Sparse Attention", Description: "Разреженное внимание MiniMax H3.", ToggleParameter: "video_sparse_attention", RequiredClasses: []string{"H3SparseAttentionAdvanced"}, Order: 70},
			{ID: "clear_vram", Name: "Поэтапная очистка VRAM", Description: "Освобождение кэша между тяжёлыми стадиями.", ToggleParameter: "video_clear_vram", RequiredClasses: []string{"LCVRAMCacheClear"}, Order: 80},
			{ID: "audio_reference", Name: "Аудиореференс", Description: "Обрезка отдельного аудиореференса.", Conditions: []workflowCondition{{Field: "input_audio", Operator: "not_empty", Value: true}}, RequiredClasses: []string{"LoadAudio", "TrimAudioDuration"}, Order: 90},
			{ID: "video_reference", Name: "Видеореференс", Description: "Загрузка фрагмента видео и при необходимости его звука.", Conditions: []workflowCondition{{Field: "input_video", Operator: "not_empty", Value: true}}, RequiredClasses: []string{"VHS_LoadVideo"}, Order: 100},
			{ID: "color_match", Name: "ColorMatch", Description: "Приведение палитры к первому фото.", ToggleParameter: "video_color_match", RequiredClasses: []string{"LCColorMatch"}, Order: 110},
			{ID: "rife", Name: "RIFE", Description: "Интерполяция кадров для повышения FPS.", ToggleParameter: "video_rife_enabled", RequiredClasses: []string{"RIFE VFI"}, Order: 120},
			{ID: "rtx", Name: "RTX Super Resolution", Description: "Финальный NVIDIA-апскейл ролика.", ToggleParameter: "video_rtx_enabled", RequiredClasses: []string{"RTXVideoSuperResolution"}, Order: 130},
			{ID: "sharpen", Name: "Image Sharpen", Description: "Финальная резкость после остальных стадий.", ToggleParameter: "video_sharpen_enabled", RequiredClasses: []string{"ImageSharpenKJ"}, Order: 140},
		},
		...
		PromptAssistant: workflowAssistantManifest{
			...
			ModeRules: map[string][]string{
				...
				"references": {
					...
					"Вернуть разделы subject_definitions, summary, retention_analysis, detailed_description, overall_soundscape и non_diegetic_music.",
...
func miniMaxH3WorkflowParameters() []workflowParameterManifest {
	return []workflowParameterManifest{
		...
		booleanWorkflowParameter("video_low_vram_attention", "VideoLowVRAMAttention", "Low VRAM Attention", "optimization", false),
		integerWorkflowParameter("video_low_vram_head_chunks", "VideoLowVRAMHeadChunks", "Группы голов attention", "optimization", 4, 1, 56, 1),
		booleanWorkflowParameter("video_chunk_feed_forward", "VideoChunkFeedForward", "Chunk FeedForward", "optimization", false),
		integerWorkflowParameter("video_chunk_feed_forward_chunks", "VideoChunkFFChunks", "Блоки FeedForward", "optimization", 2, 1, 64, 1),
		integerWorkflowParameter("video_chunk_feed_forward_threshold", "VideoChunkFFThreshold", "Порог токенов FeedForward", "optimization", 4096, 256, 262144, 256),
