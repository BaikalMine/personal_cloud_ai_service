...
func databaseTablePolicies() map[string]databaseTablePolicy {
	...
	return map[string]databaseTablePolicy{
		...
		"user_notification_preferences":    {Label: "Настройки уведомлений", Owner: "Профиль пользователя", Retention: fixed("До удаления аккаунта")},
		"user_notification_revision":       {Label: "Ревизии уведомлений", Owner: "Система", Retention: fixed("До удаления аккаунта")},
		...
		"service_observations":             {Label: "Задержки зависимостей", Owner: "Наблюдаемость", Retention: duration(func(p config.RetentionPolicy) time.Duration { return p.HostMetrics }), Configuration: "HOST_METRIC_RETENTION", Managed: true},
		"gateway_observations":             {Label: "Снимки Gateway", Owner: "Наблюдаемость", Retention: duration(func(p config.RetentionPolicy) time.Duration { return p.HostMetrics }), Configuration: "HOST_METRIC_RETENTION", Managed: true},
		...
		"prompt_assistant_runs":            {Label: "Качество промт-ассистента", Owner: "Промт-ассистент", Retention: fixed("Вместе с AI-контентом"), Managed: true},
		...
		"content_media_chunks":             {Label: "Зашифрованные блоки медиа", Owner: "Контроль контента", Retention: fixed("Вместе с медиа"), Managed: true},
