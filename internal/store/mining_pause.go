...
func (s *Store) CreateQuickGenerationMiningLease(ctx context.Context, lease domain.QuickGenerationMiningLease) error {
	_, err := s.db.ExecContext(ctx, `
		INSERT INTO quick_generation_mining_leases
			(id, prompt_id, user_id, miner_id, script_path, process_name, resume_mining, generation_job_id)
		VALUES ($1, NULLIF($2,''), $3, $4, $5, $6, $7, NULLIF($8,0))
	`, lease.ID, lease.PromptID, lease.UserID, lease.MinerID, lease.ScriptPath, lease.ProcessName, lease.ResumeMining, lease.GenerationJobID)
	return err
}
...
func (s *Store) ActiveQuickGenerationMiningLease(ctx context.Context) (domain.QuickGenerationMiningLease, error) {
	return scanQuickGenerationMiningLease(s.db.QueryRowContext(ctx, `
		SELECT id, COALESCE(prompt_id,''), COALESCE(generation_job_id,0), user_id, miner_id, script_path, process_name, resume_mining, created_at
...
func (s *Store) DeleteQuickGenerationMiningLease(ctx context.Context, leaseID string) (domain.QuickGenerationMiningLease, int, error) {
	...
	lease, err := scanQuickGenerationMiningLease(tx.QueryRowContext(ctx, `
		...
		RETURNING id, COALESCE(prompt_id,''), COALESCE(generation_job_id,0), user_id, miner_id, script_path, process_name, resume_mining, created_at
	`, leaseID))
	if err != nil {
		return domain.QuickGenerationMiningLease{}, 0, err
	}
...
func scanQuickGenerationMiningLease(row quickGenerationMiningLeaseScanner) (domain.QuickGenerationMiningLease, error) {
	...
	err := row.Scan(&lease.ID, &lease.PromptID, &lease.GenerationJobID, &lease.UserID, &lease.MinerID, &lease.ScriptPath, &lease.ProcessName, &lease.ResumeMining, &lease.CreatedAt)
