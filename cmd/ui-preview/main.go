	...
	loraJobs := []map[string]any{
		{
			...
			"Stage": "Обучаем LoRA", "Progress": 61, "Message": "Шаг 976 из 1600. Задание использует GPU, майнинг приостановлен.", "CanCancel": true, "CanDownload": false, "CanDelete": false, "CreatedAt": now.Add(-46 * time.Minute),
		},
		{
			...
			"Stage": "Готово", "Progress": 100, "Message": "LoRA установлена в ComfyUI и готова к использованию.", "CanCancel": false, "CanDownload": true, "CanDelete": true, "ArtifactName": "product_light_v2.safetensors", "CreatedAt": now.Add(-7 * time.Hour),
		},
	}
	loraAPIJobs := []map[string]any{
		{"id": "lora-preview-running", "name": "Редакционный портрет", "output_name": "editorial_portrait_v1", "profile_id": "krea2-raw-official-bf16", "family": "krea2", "family_label": "Krea2", "base_model": "krea2Raw_v10.safetensors", "state": "running", "state_label": "Обучение", "state_class": "is-active", "stage": "Обучаем LoRA", "progress": 61, "message": "Шаг 976 из 1600. Задание использует GPU, майнинг приостановлен.", "sample_count": 24, "concept_label": "Персонаж", "preset_label": "Основной", "resolution": 768, "max_train_steps": 1600, "can_cancel": true, "can_delete": false, "cancel_url": "/train-lora/lora-preview-running/cancel", "created_at": now.Add(-46 * time.Minute).UnixMilli(), "updated_at": now.Add(-4 * time.Second).UnixMilli()},
		{"id": "lora-preview-complete", "name": "Свет предметной съёмки", "output_name": "product_light_v2", "profile_id": "flux2-klein-9b-pornmaster-v4-base", "family": "flux2-klein", "family_label": "Flux.2 Klein", "base_model": "pornmasterFlux2Klein_v4BaseBf16.safetensors", "state": "completed", "state_label": "Готово", "state_class": "is-complete", "stage": "Готово", "progress": 100, "message": "LoRA установлена в ComfyUI и готова к использованию.", "sample_count": 38, "concept_label": "Стиль", "preset_label": "Детальный", "resolution": 1024, "max_train_steps": 2800, "can_download": true, "can_delete": true, "download_url": "/train-lora/lora-preview-complete/download", "delete_url": "/train-lora/lora-preview-complete/delete", "artifact_name": "product_light_v2.safetensors", "created_at": now.Add(-7 * time.Hour).UnixMilli(), "updated_at": now.Add(-6 * time.Hour).UnixMilli()},
	}
