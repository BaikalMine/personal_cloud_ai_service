...
test("draft restores prompt, LoRA, seed, optional settings and manual assistant edits", async ({ page }) => {
  await open(page);
  await page.getByRole("button", { name: /TXT Текст в изображение/ }).click();
  ...
  await page.locator('input[name="seed"]:visible').fill("123456");
  ...
  await expect(page.locator('input[name="seed"]:visible')).toHaveValue("123456");
...
test("draft restores all four REF2VA references and their roles", async ({ page }) => {
  await open(page);
  await page.getByRole("button", { name: /VID Видео/ }).click();
  ...
  await expect(page.getByRole("radio", { name: /^REF2VA/ })).toBeChecked();
  for (let index = 1; index <= 4; index += 1) {
  ...
  await expect(page.locator("#workflow-next")).toHaveText("Продолжить");
});
...
test("two tabs resolve conflicts explicitly and owner can delete the draft", async ({ page, context }) => {
  ...
  await expect(page.locator("#positive-prompt")).toHaveValue("Changed on second tab");
  await page.reload();
  await expect(page.getByRole("heading", { name: "Что будем делать?", exact: true })).toBeVisible();
  await other.close();
});
